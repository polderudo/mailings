...
//https://github.com/caffix/queue/blob/master/queue.go
...
// CloneData clones the data and returns it ordered by priority
func (q *queue) CloneData() []*QueueElement {
	q.Lock()
	defer q.Unlock()
	var ppq priorityQueue
	var ret []*QueueElement
	...
	sort.Sort(ppq)
	for i := 0; i < ppq.Len(); i++ {
		ret = append(ret, &QueueElement{
			Data:     q.pq[i].Data,
			priority: q.pq[i].priority,
			index:    q.pq[i].index,
		})
	}
	return ret
