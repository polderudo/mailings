...
type MailingTableRow struct {
	...
	TotalRecipients string `json:"totalRecipients"`
...
func LoadMailingTableRows(ctx context.Context, request FilterRequest[map[string]string]) ([]MailingTableRow, int64, error) {
	...
	rows, err := StdDB.QueryContext(
		ctx,
		`SELECT m.id, m.name, t.name, l.name, d.domain, m.status, m.total_recipients, m.sent_count
FROM mailing m
...
JOIN mail_domain d   ON d.id = m.domain_id`+
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &name, &tName, &lName, &domain, &status, &total, &sent); err != nil {
			return nil, 0, err
		}
		result = append(result, MailingTableRow{
			...
			Domain:          domain,
			Status:          status,
			TotalRecipients: fmt.Sprintf("%d", total),
